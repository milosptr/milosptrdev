...
export const h2 = function H2(props) {
  ...
  let hasTitle = true
  try {
    hasTitle = props.children.props.children !== '/'
  } catch (e) {
    hasTitle = false
  }
...
function ContentWrapper({ className, children }) {
  return (
    <div className='mx-auto max-8xl px-6 lg:flex lg:px-12'>
      <div className='lg:ml-96 lg:flex lg:w-full lg:justify-end lg:pl-32'>
        <div className={twMerge('mx-auto max-w-lg lg:mx-0 lg:w-0 lg:max-w-4xl lg:flex-auto', className)}>
          {children}
        </div>
      </div>
    </div>
...
function ArticleHeader({ id, date }) {
  return (
    <header className='relative mb-10 xl:mb-0'>
      <div className='pointer-events-none absolute left-[max(-0.5rem,calc(50%-18.625rem))] top-0 z-50 flex h-4 items-center justify-end gap-x-2 lg:left-0 lg:right-[calc(max(2rem,50%-28rem)+40rem)] lg:min-w-[32rem] xl:h-8'>
        {!!date && (
          <>
            <Link
              href={`#${id}`}
...
export const article = function Article({ id, title, spacing, date, children }) {
  let { isFeed } = useFeed()
  let heightRef = useRef()
  let [heightAdjustment, setHeightAdjustment] = useState(0)
  const noSpacing = spacing && spacing === 'none'
  ...
  return (
    <article
      id={id}
      className={`${noSpacing ? 'mt-[1.5rem!important]' : 'scroll-mt-16'}`}
